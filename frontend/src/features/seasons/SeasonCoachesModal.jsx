...
import coachSeasonService from './coachSeasonService';
...
export default function SeasonCoachesModal({ isOpen, season, onClose, onSaved, canManage }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const requests = [coachSeasonService.listSeasonCoaches(season.id)];

      if (canManage) {
        requests.push(groupService.listCoachDirectory({ limit: 100, offset: 0 }));
      }

      const [assignedResponse, directoryResponse] = await Promise.all(requests);
  ...
  async function handleAssignCoach() {
    if (!selectedCoachId || !season?.id || !canManage) {
      return;
    }
    ...
    try {
      ...
      await coachSeasonService.assignCoachToSeason({
        seasonId: Number(season.id),
        coachId: Number(selectedCoachId),
      });

      setSelectedCoachId('');
      setSuccessMessage('Треньорът е назначен към сезона.');
      await loadData();
      if (onSaved) {
        await onSaved();
      }
  ...
  async function handleUnassignCoach() {
    ...
    try {
      ...
      await coachSeasonService.unassignCoachFromSeason(Number(season.id), Number(coach.id));

      setConfirmState({ isOpen: false, coach: null });
      setSuccessMessage('Треньорът е премахнат от сезона.');
      await loadData();
      if (onSaved) {
        await onSaved();
      }
  ...
  return (
    <>
      <FormModal
        title={season ? `Треньори в сезон: ${season.name}` : 'Треньори в сезон'}
        isOpen={isOpen}
        onClose={onClose}
        footer={
          <>
      ...
      >
        <div className="academy-coaches-modal-stack">
          ...
          {canManage && !isLoading && !assignableCoachOptions.length ? (
            <Alert type="info">Всички налични треньори вече са назначени към този сезон.</Alert>
          ) : null}
          ...
          {!isLoading && !(assignedCoaches || []).length ? (
            <Alert type="info">Все още няма назначени треньори към този сезон.</Alert>
          ) : null}
        </div>
      </FormModal>

      <ConfirmDialog
        title="Премахване на треньор от сезон"
        message={
          confirmState.coach
            ? `Сигурни ли сте, че искате да премахнете ${formatCoachName(confirmState.coach)} от този сезон?`
            : 'Сигурни ли сте, че искате да премахнете треньора от този сезон?'
        }
